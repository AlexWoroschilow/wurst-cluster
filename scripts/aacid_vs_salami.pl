...
sub main ($ $ $ $) {
	...
	for my $structure1 (@structures) {
		...
		next if( not $hashtable_cluster_aacid{$structure1} 
			or not $hashtable_cluster_salami{$structure1});

		for my $structure2 (@structures) {

			next if ( not $hashtable_cluster_aacid{$structure2} 
			...
			my $in_both_clusters = ($structure1_cluster_salami == $structure2_cluster_salami) ? 1 : 0;
